/****************************************************************//**
* \file 	SendMessageServlet.java
* \author 	
* \version 	1.0
* \date 	11.2012
*
* \brief 	Servlet that sends a message to a device.
* \details 	
********************************************************************/
...
import com.appspot.drivenotification.model.ApiKeyInitializer;
...
public class SendMessageServlet extends BaseServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (regId != null) {
      sendSingleMessage(regId, resp, message,latitude,longitude);
      return;
    }
  ...
  /****
   * 
   * @param regId device register ID.
   * @param resp Http response.
   * @param messagestr Message to be sent.
   * @param latitude 
   * @param longitude
   */
  private void sendSingleMessage(String regId, HttpServletResponse resp,String messagestr,String latitude,String longitude) {
    logger.info("Sending message to device " + regId);
    Message message = new Message.Builder().addData("message", messagestr).addData("latitude", latitude)
  ...
  private void sendMulticastMessage(String multicastKey,
      HttpServletResponse resp , String messagestr) {
    // Recover registration ids from datastore
    List<String> regIds = Datastore.getMulticast(multicastKey);
    Message message = new Message.Builder().addData("message", messagestr).build();
